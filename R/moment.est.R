                       ...
                       start.cov = NULL, parallel=FALSE, verbose=FALSE)
{
    ...
    if(parallel) {
      ...
      wtot2 <- Reduce('+', lapply(results, function(x) x[[1]]))/length(results)
      wt.cov <- Reduce('+', lapply(results, function(x) x[[2]]))/length(results)
      rm(results); gc()
    }
