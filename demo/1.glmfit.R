...
start <- NULL ;etastart <- NULL ; mustart <- NULL;intercept <- TRUE;
...
for (dataid in seq_len(nfiles)){
  ...
  saveRDS(list(nobs = nobs, weights = weights, offset = offset, y = y, group = group),file = infofilename)
  ...
  (R[rank.fixed + r1.random, rank.fixed + r1.random] <- R.random[r1.random, r1.random])
  ...
  write.table(estdispersion ,file = dispfilename, sep = ",", quote = FALSE, col.names = FALSE,row.names = FALSE)
  ...
  if(control$parallel) {
    ...
    subspace <- as.list(rep(NULL,m$ngroups))
    precision <- as.list(rep(NULL, m$ngroups))
    for (i in seq_len(m$ngroups)) {
      if (nrow(Rp[[i]]) > 0L) {
	prec.sqrt <- backsolve(R, t(Rp[[i]][,pivot[r1],drop=FALSE]),
			       transpose=TRUE)
  ...
  subspacefilename <- paste(datafile_folder,'subspace.',dataid,'.rds',sep = "")
...
dispfiles <- grep('disp',list.files(datafile_folder,full.names = TRUE),value = TRUE)
...
} else { disp.summary['dispersion.sum']/disp.summary['ngroups']}
