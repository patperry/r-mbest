...
gen.replicate <- function 
( ngroup, nobs, nobs.dispersion,
 ...
 dispersion, family
	...
	ranef.cov <- diag(diag(ranef.scale^2
			       * solve(rWishart(1, ranef.cov.df, diag(nranef))[,,1])) ,nrow = nranef)
...
r <- gen.replicate(ngroup=10,  nobs=10000, nobs.dispersion=1, nfixef=5,
		   fixef.mean.df=4, fixef.scale=1, nranef=5, ranef.cov.df=10,
		   ranef.scale=1, dispersion=1, family=family)
...
for(i in seq_len(length(subsets))){
  ...
  saveRDS(list(x = r$x[idx,],z = r$z[idx,],y = r$y[idx],group = factor(r$group[idx])),file = data_filename)
}
