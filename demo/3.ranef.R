...
for(dataid in seq_len(nfiles)){
  ...
  ranef.sep <- moment.est.cov.mapper(s$coef, rank.fixed, s$subspace, s$precision, dispersion, 
				     start.cov , diagcov = control$diagcov, mean = est.mean$mean)
...
#---------- 
# estimate ranef cov
raneffiles <- grep('ranef.*.rds',list.files(datafile_folder,full.names = TRUE),value = TRUE)
cov.info <- as.list(rep(NULL,nfiles))
